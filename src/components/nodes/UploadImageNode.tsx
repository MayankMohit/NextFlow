...
import { Image, Upload, Loader2, Play, X } from 'lucide-react'
...
const borderColor = selected ? NODE_COLOR : isDark ? '#2a2a2a' : '#e0e0e0'
  ...
  const glowKeyframes = `
    @keyframes upload-image-node-glow {
      0%, 100% {
  ...
  const glowStyle: React.CSSProperties = isNodeRunning
    ? {
        ...
        animation: 'upload-image-node-glow 1.8s ease-in-out infinite',
      }
    : {
        borderColor,
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      updateNodeData(id, { imageUrl: blob.url })
    ...
    <div className="relative" onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
      ...
      <div className={`w-64 ${nodeBg} border ${border} rounded-lg overflow-hidden`} style={glowStyle}>
        <div className={`flex items-center gap-2 px-3 py-2 border-b ${hdrBorder}`}>
          <Image size={13} style={{ color: NODE_COLOR }} />
          <span className={`text-xs font-medium ${textMain}`}>Upload Image</span>
        </div>
        <div className="p-3">
          {imageUrl ? (
            <div className="relative">
              ...
              <button onClick={() => updateNodeData(id, { imageUrl: null })}
            ...
            <label className={`flex flex-col items-center justify-center w-full h-32 border border-dashed rounded transition-colors ${dropBorder}`}>
              {uploading ? (
                ...
                <><Upload size={20} className={`${iconColor} mb-1`} /><span className={`text-xs ${iconColor}`}>Click to upload</span><span className={`text-xs mt-0.5 ${subText}`}>jpg, png, webp, gif</span></>
              )}
