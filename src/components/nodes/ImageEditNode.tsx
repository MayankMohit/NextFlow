...
import { Wand2, Play, Loader2, X } from "lucide-react";
import { useWorkflowStore } from "@/store/workflowStore";
import { useNodeHover } from "@/hooks/usenodehover";
import { useNodeStatus } from "@/hooks/useNodeStatus";
import OutputPreview from "./shared/OutputPreview";

const NODE_COLOR = "#f43f5e";

export default function ImageEditNode({ selected, data, id }: NodeProps) {
  ...
  const glowStyle: React.CSSProperties = isNodeRunning
    ? {
        ...
        animation: "imageedit-node-glow 1.8s ease-in-out infinite",
      }
    : {
        borderColor,
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Wand2 size={13} style={{ color: NODE_COLOR }} />
        ...
        <div className="p-3 flex flex-col gap-2">
          ...
          <div className="flex items-center gap-2 h-4">
            ...
            <span className={`text-xs truncate ${hintColor}`}>
              {isConnected("image_url") ? "connected" : "connect an image"}
            </span>
          </div>
          ...
          <p className={`text-xs ${hintColor}`}>
            flux-2-klein · describe the change you want
          </p>
        </div>
        ...
        <Handle
          ...
          position={Position.Left}
          id="prompt"
          style={{ top: 79 + handleOffset, ...tHandle }}
        />
        <Handle
          type="source"
